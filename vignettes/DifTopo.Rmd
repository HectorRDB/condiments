...
Another way to look at this is to consider how the cells of one condition differentiate between two lineages, compared to other conditions. For this, we rely on the classifier test. The idea is that, if two conditions are identical, the accuracy of any classifier will never be higher than chance. This method is very versatile and can accommodate more than two conditions, and multivariate distribution (which is the case for the weights vectors when there is more than one lineage).

```{r}
set.seed(12)
diffDifferentiationTest(sds, conditions = condition, global = FALSE, 
                        pairwise = TRUE, thresh = 0)
```
