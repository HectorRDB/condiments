...
__slingshot__[@Street2018a] relies on a reduced dimensionality reduction representation of the data, as well as on cluster labels. We can vizualise those below: 
...
The test clearly fails to reject the null that we can fit a common trajectory so we can continue with the `sds` object. This will facilitate downstream analysis. For an example of how to proceed if the __topologyTest__, we invite the user to refer to [one of the case studies used in our paper](https://hectorrdb.github.io/condimentsPaper/articles/KRAS.html).
