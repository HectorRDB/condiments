...
test_that("The diffProgressionTest work on expected inputs",{
  ...
  set.seed(23)
  test <- diffProgressionTest(sds = sds, conditions = condition, rep = 2)
  expect_is(test, "data.frame")
  expect_equal(dim(test), c(1, 3))
  expect_equal(colnames(test),  c("lineage", "statistic", "p.value"))
  set.seed(23)
  test_all <- diffProgressionTest(sds = sds, conditions = condition, rep = 2, lineage = TRUE)
  expect_equal(nrow(test_all), length(slingCurves(sds)) + 1)
  expect_equal(test[, 2:3], test_all[1, 2:3])
  set.seed(23)
  test_lineages <- diffProgressionTest(sds = sds, conditions = condition, rep = 2,
  ...
  set.seed(12)
  test_sce <- diffProgressionTest(sds = sce, conditions = "cond", rep = 2)
  expect_identical(test_sce, test)
})

test_that("The diffProgressionTest work on all tests", {
  set.seed(23)
  test <- diffProgressionTest(sds = sds, conditions = condition, rep = 2,
                              method = "Classifier")
  expect_is(test, "data.frame")
