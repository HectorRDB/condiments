...
test_that("The diffTopoTest work on expected inputs",{
  ...
  expect_is(test, "htest")
  expect_equal(test$method, "Two-sample Weighted Kolmogorov-Smirnov test with threshold 0.05")
  expect_equal(test$alternative, "Infer separate trajectories for each condition")
  ...
  expect_is(test, "htest")
  expect_equal(test$method, "Two-sample Weighted Kolmogorov-Smirnov test with threshold 0")
